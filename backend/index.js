...
const app = express()
const { PORT, PATH } = require('./utils/config')
...
app.get('/api/:name', (req, res) => {
  ...
  if (package) {
    const dependants = packages.filter(ifDependenciesInclude(name)).map(getName)
    ...
    res.status(404).json({ error: `No package with name ${name} found!` })
  }
})
